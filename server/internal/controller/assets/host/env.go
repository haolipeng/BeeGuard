...
// EnvHandler 可疑环境变量资产处理器
type EnvHandler struct {
	DB *gorm.DB
}

// ListEnvs 获取可疑环境变量列表（支持搜索查询）
func (h *EnvHandler) ListEnvs(c *gin.Context) {
	...
	var envs []host.EnvSuspicious
	var total int64

	query := h.DB.Model(&host.EnvSuspicious{})
	...
	if varName != "" {
		query = query.Where("var_name LIKE ?", "%"+varName+"%")
	}
	...
	result = query.Order("created_at DESC").Limit(limit).Offset(offset).Find(&envs)
	if result.Error != nil {
