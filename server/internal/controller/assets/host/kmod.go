...
// KmodHandler 内核模块资产处理器
type KmodHandler struct {
	DB *gorm.DB
}

// ListKmods 获取内核模块资产列表（支持搜索查询）
func (h *KmodHandler) ListKmods(c *gin.Context) {
	...
	result = query.Order("created_at DESC").Limit(limit).Offset(offset).Find(&kmods)
	if result.Error != nil {
