...
// DatabaseHandler 数据库资产处理器结构体，用于处理与数据库资产相关的HTTP请求
type DatabaseHandler struct {
	// DB 数据库连接实例
	DB *gorm.DB
}

// ListDatabases 获取数据库资产列表（支持搜索查询）
func (h *DatabaseHandler) ListDatabases(c *gin.Context) {
	...
	var databases []host.Database
	var total int64

	// 构建查询条件
	query := h.DB.Model(&host.Database{})
	...
	if dbType != "" {
		query = query.Where("db_type LIKE ?", "%"+dbType+"%")
	}
	if dbVersion != "" {
		query = query.Where("db_version LIKE ?", "%"+dbVersion+"%")
	}
	if portStr != "" {
	...
	// 分页查询数据，按创建时间倒序排列
	result = query.Order("created_at DESC").Limit(limit).Offset(offset).Find(&databases)
	if result.Error != nil {
