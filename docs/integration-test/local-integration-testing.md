...
**重要**：`/opt/cloudsec/hcids/conf/server.yaml` 默认的数据库配置指向远程服务器，本地集成测试需修改为本地 PostgreSQL。
...
> **测试完成后务必恢复原配置**：`cp /opt/cloudsec/hcids/conf/server.yaml.bak /opt/cloudsec/hcids/conf/server.yaml`
...
**移除白名单方法**：编辑 `/opt/cloudsec/hcids/conf/server.yaml`，找到 `object_name: ssh` 对应的 task，将 `data` 字段中的 `"whitelist":["127.0.0.1","::1"]` 改为 `"whitelist":[]`，然后重启 hcids 和 Agent 使配置生效。
