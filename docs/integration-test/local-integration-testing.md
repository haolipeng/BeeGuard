...
**验证**：执行 `ls -la /opt/cloudsec/bin/hcids /opt/cloudsec/conf/server.yaml`，两个文件都存在即成功。

部署后 `/opt/cloudsec/` 中 hcids 相关目录结构：
...
编辑 `/opt/cloudsec/conf/server.yaml`，确认数据库连接信息：

```yaml
server:
  port: 50051          # gRPC 端口
  http_port: 8081      # HTTP API 端口

database:
  host: 127.0.0.1      # 本地 PostgreSQL
  port: 5432
  user: postgres
  password: "root"
  database: soc
...
# 测试环境可关闭非必要模块
geoip:
  enabled: false       # 无 GeoIP 数据库时设为 false
vuln:
  enabled: false       # 无 Trivy 数据库时设为 false
```
...
编辑 `/opt/cloudsec/agent.yaml`，确认 server 地址指向 hcids：

```yaml
server: "<HCIDS_HOST>:50051"     # hcids gRPC 地址
connect_timeout: 30
...
> 本机测试时 server 设为 `127.0.0.1:50051`
...
```bash
# 终端 C：访问已知挖矿域名（DNS 查询即触发，无需实际连通）
