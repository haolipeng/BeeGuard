...
**移除白名单方法**：编辑远程服务器的 `/opt/cloudsec/conf/server.yaml`，找到 `object_name: ssh` 对应的 task，将 `data` 字段中的 `"whitelist":["127.0.0.1","::1"]` 改为 `"whitelist":[]`，然后重启远程 hcids 和本地 Agent 使配置生效。
