...
| vuln (hcids) | - | vuln_info | UPSERT |
| vuln (hcids) | - | host_vuln_scan_task | INSERT |
| vuln (hcids) | - | host_vuln_detail | INSERT |
| vuln (hcids) | - | image_vuln_scan_task | INSERT |
| vuln (hcids) | - | image_vuln_detail | INSERT |
...
| nc (netcat) | `apt install netcat-openbsd` | 5.3 反弹 Shell 测试 |
...
- `dialing server` 后无报错、出现 `forwarding task to plugin` → Agent 与远程 hcids 连接成功
...
> **⚠ 必须先移除白名单，否则本地测试无法触发告警！** 默认配置中 `127.0.0.1` 和 `::1` 在白名单内，从本地发起的 SSH 登录失败不会被检测。

**移除白名单方法**（需登录远程服务器操作）：

1. 编辑远程服务器的 `/opt/cloudsec/conf/server.yaml`
2. 找到 `object_name: ssh` 对应的 task
3. 将 `data` 字段中的 `"whitelist":["127.0.0.1","::1"]` 改为 `"whitelist":[]`
4. 重启远程 hcids：`sudo systemctl restart hcids`
5. 重启本地 Agent（Agent 启动时从 hcids 拉取最新配置）

> **验证白名单已生效**：重启后在 Agent 日志中搜索 `ssh` 相关的 task 下发日志，确认 `whitelist` 字段为空数组。
...
#### 前置条件二：处理远程 hcids 服务端配置覆盖

远程 hcids 的 `server.yaml` 中 `ssh_anomaly_login` 任务默认配置为 `"enabled":false,"anomaly_rules":[]`。Agent 连接后，服务端会自动下发此配置，**覆盖本地配置**。

**方案 A（推荐）：修改远程 hcids 的 server.yaml**
...
修改后重启远程 hcids：`sudo systemctl restart hcids`
...
通过 curl 调用远程 hcids 的 `POST /api/baseline/check` 下发任务：
...
1. **Trivy 漏洞数据库**：远程 hcids 启动时会自动从 OCI 仓库下载 Trivy DB（首次下载约 40MB），需确保远程服务器可访问 `ghcr.io`。
...
2. **远程 server.yaml 漏洞模块配置**：确认远程 hcids 的 `vuln.enabled` 为 `true`（默认已启用）。
...
等待首次匹配完成后（远程 hcids 日志出现 `漏洞匹配任务完成`），查询远程数据库。
...
```
Collector Plugin                  远程 hcids Server                  远程 PostgreSQL
┌──────────────────┐  gRPC       ┌─────────────────┐  UPSERT        ┌──────────────────────┐
...
2. **远程 server.yaml 中启用镜像和镜像软件包采集任务**：默认 `tasks` 中**未配置** DataType 5058 和 5059，需在远程服务器上手动添加。
   ...
   在远程 hcids 的 `server.yaml` 的 `tasks` 列表末尾追加：
   ...
   > **重要**：修改后需重启远程 hcids 和本地 Agent 使配置生效。
...
3. 远程 hcids 启动 → 漏洞模块初始化
4. 本地启动 Agent → Collector 采集镜像信息和镜像内软件包
5. 远程 hcids 启动约 **30 秒**后自动执行漏洞匹配（含镜像匹配）
