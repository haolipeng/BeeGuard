...
本文档描述通过 hcids HTTP API 远程卸载 Agent 的端到端测试流程：调用卸载 API → hcids 通过 gRPC 下发卸载命令（DataType 1061）→ Agent 生成卸载脚本并退出 → 脚本完成清理。
...
```
HTTP Client              hcids Server              Agent                          卸载脚本
    │                         │                       │                               │
...
- hcids Server 已部署（`/opt/cloudsec/hcids/`）
...
确保 `/opt/cloudsec/hcids/conf/server.yaml` 的 `database` 部分指向本地：
...
确保 `/opt/cloudsec/agent/agent.yaml` 的 `server` 指向本地 hcids：
...
安装完成后，确认 agent.yaml 中 `server` 为 `127.0.0.1:50051`（install.sh 可能使用 hcids 配置中的远程地址，需手动修正）：
...
1. 检查 Agent 日志是否收到卸载命令：
   ...
   - 无 `received uninstall command` → Agent 与 hcids 的 gRPC 连接异常
...
```bash
# 停止 hcids（Terminal A 按 Ctrl+C）

# 清理残留的卸载脚本
rm -f /tmp/cloudsec-uninstall-*.sh

# 恢复 hcids 配置（如果修改过数据库配置）
cp /opt/cloudsec/hcids/conf/server.yaml.bak /opt/cloudsec/hcids/conf/server.yaml 2>/dev/null
```
