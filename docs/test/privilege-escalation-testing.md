...
本文档描述 Driver 插件本地提权检测功能的测试方法，面向技术研发人员。

---

## 1. 概述

Driver 插件通过 eBPF Hook `kprobe/commit_creds` 检测进程的 uid/euid 提权行为。
...
| 文件 | 说明 |
|------|------|
| `business_plugins/driver/ebpf/bpf/hids.bpf.c` | eBPF 内核代码（commit_creds hook） |
| `business_plugins/driver/main.go` | 事件处理逻辑 |
| `business_plugins/driver/config/privilege_escalation_whitelist.yaml` | 白名单配置 |
...
```bash
sudo ./build/agent -standalone -plugins=driver -output=stderr -test 2>&1 | tee driver.log
```
...
| `-output=stderr` + tee 重定向 | 查看保存的日志文件：`grep "Privilege escalation" driver.log` |
...
```bash
# 从 tee 保存的日志中搜索
grep "Privilege escalation" driver.log
