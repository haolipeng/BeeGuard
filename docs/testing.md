...
| 集成测试 | Agent + hcids Server 联合验证，数据库端到端 | [手动执行](integration-test/integration-testing.md) |

### 测试覆盖范围

| 插件 | 单元测试 | E2E 测试 | 手动验证 | 集成测试 |
|------|---------|---------|---------|---------|
| collector | port_test.go | [自动化](standalone-test/collector-testing.md) | - | [数据库验证](integration-test/integration-testing.md#四collector-插件测试) |
| baseline | - | [E2E 测试](standalone-test/baseline-testing.md) | - | [数据库验证](integration-test/integration-testing.md#七baseline-插件测试) |
| detector | - | - | 需手动验证 | [数据库验证](integration-test/integration-testing.md#六detector-插件测试) |
| ebpf_base_detector | - | - | 需手动验证 | [数据库验证](integration-test/integration-testing.md#五ebpf_base_detector-插件测试) |
| scanner | - | [E2E 测试](standalone-test/scanner-testing.md) | - | [数据库验证](integration-test/integration-testing.md#八scanner-插件测试) |
| nids | - | [手动验证](standalone-test/nids-testing.md) | - | [数据库验证](integration-test/integration-testing.md#九nids-插件测试) |
...
# 2. 确认插件已部署
ls -la /opt/cloudsec/agent/plugins/detector/
```
...
ssh:
  ...
  rules:
    - name: auth_failure_brute_force
      ...
      group_by: source_ip
EOF
# 将此配置复制到 /opt/cloudsec/agent/plugins/detector/config/rules/ssh_brute_force.yaml
...
```bash
# 查看/编辑规则配置
cat /opt/cloudsec/agent/plugins/detector/config/rules/ssh_anomaly_login.yaml
```
...
## 三、ebpf_base_detector 插件手动验证

ebpf_base_detector 插件基于 eBPF，**必须使用 root 权限**运行。
...
# 3. 确认部署
ls -la /opt/cloudsec/agent/plugins/ebpf_base_detector/
ls -la /opt/cloudsec/agent/plugins/ebpf_base_detector/config/
```
...
| 配置文件 | 路径 | 用途 |
|---------|------|------|
| 高危命令规则 | `/opt/cloudsec/agent/plugins/ebpf_base_detector/config/dangerous_commands.yaml` | 定义 4 条高危命令检测规则 |
| 可信任程序白名单 | `/opt/cloudsec/agent/plugins/ebpf_base_detector/config/privilege_escalation_whitelist.yaml` | 本地提权检测白名单（sudo、su 等） |
...
| 60 | Connect 出站连接事件 | 进程发起网络连接 | DEBUG |
| 6006 | 本地提权告警 | 检测到非法提权 | WARN |

### 3.2 Standalone 模式说明

Standalone 模式允许不连接 gRPC Server 进行本地测试，检测结果输出到日志或文件。**在测试 ebpf_base_detector 插件时，推荐优先使用此模式。**

**Standalone 配置（在 agent.yaml 中）:**
...
- **检测事件输出**: 由 `-output` 参数控制，输出到 stderr 或指定文件
- **Agent 运行日志**: 输出到当前目录下的 `logs/agent.log`
- **日志格式**: 结构化日志（zap），控制台格式
- **日志级别**: INFO（高危命令）、WARN（本地提权）、ERROR（错误）
- **保存检测事件**: 可通过重定向保存到文件
  ```bash
  cd /opt/cloudsec/agent
  sudo ./bin/agent -standalone -plugins=ebpf_base_detector -output=/opt/cloudsec/agent/logs/agent.log -test 2>&1 | tee ebpf_base_detector.log
  ```
...
INFO  Dangerous command detected
    rule_id=2001  rule_name=危险删除操作  severity=critical
...
**预期日志:**
```
INFO  Dangerous command detected  rule_id=2001  rule_name=危险删除操作  severity=critical
```
...
**预期日志:**
```
INFO  Dangerous command detected  rule_id=2002  rule_name=敏感文件访问  severity=high
```
...
**预期日志:**
```
INFO  Dangerous command detected  rule_id=2003  rule_name=危险权限修改  severity=high
```
...
**规则配置文件:**
`/opt/cloudsec/agent/plugins/ebpf_base_detector/config/dangerous_commands.yaml`
...
ebpf_base_detector 插件通过 eBPF Hook `kprobe/commit_creds` 检测进程的 uid/euid 提权行为，并在内核层进行白名单过滤。
...
**配置文件:**
`/opt/cloudsec/agent/plugins/ebpf_base_detector/config/privilege_escalation_whitelist.yaml`
