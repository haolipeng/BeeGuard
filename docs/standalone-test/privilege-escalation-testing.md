...
验证 ebpf_base_detector 插件的本地提权检测功能（DataType 60）：eBPF 通过 `kprobe/commit_creds` Hook 监控内核凭证变更，当进程的 uid/euid 从非 0 变为 0（提权到 root），且可执行文件不在内核白名单中时，产生告警。本文档选取 2 条代表性用例进行验证，覆盖自编译 SUID 程序和系统 SUID 程序复制两种场景。
