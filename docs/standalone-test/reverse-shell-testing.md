...
| 5 | nc 工具 | `nc.traditional -h 2>&1 \| head -1` | 输出包含 `netcat` |
...
每个测试需要 3 个终端：**Terminal A**（Agent，已在 Step 1 启动）、**Terminal B**（监听端）、**Terminal C**（触发端）。每执行一条后，检查 `/tmp/ebpf_test.log` 是否出现对应告警。
