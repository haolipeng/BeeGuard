...
func Load(ctx context.Context, config proto.Config) (plg *Plugin, err error) {
	...
	// 传递 HANDLER 环境变量（用于 standalone 模式下过滤 handler）
	if h := os.Getenv("HANDLER"); h != "" {
		cmd.Env = append(cmd.Env, "HANDLER="+h)
	}
	...
	if config.Name == "ebpf_base_detector" || config.Name == "nids" || config.Name == "scanner" || config.Name == "detector" {
