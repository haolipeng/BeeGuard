...
每个测试需要 3 个终端：**Terminal A**（Agent，已在 Step 1 启动）、**Terminal B**（宿主机监听端）、**Terminal C**（容器内触发端）。每执行一条后，检查 `/tmp/ebpf_test.log` 是否出现对应告警。
...
> 注意：`172.17.0.1` 是 Docker 桥接网关 IP（即宿主机在 docker0 网桥上的地址），请根据 Step 2 中实际查询结果替换。
...
> 注意：将 `172.17.0.1` 替换为 Step 2 中查询到的实际网关 IP。
...
> 注意：将 `172.17.0.1` 替换为 Step 2 中查询到的实际网关 IP。必须使用 bash 而非 dash/sh，`/dev/tcp` 是 bash 内建功能。
