...
ebpf_base_detector 插件使用 eBPF 技术，**编译机器**需要以下工具（目标部署机器不需要）：
...
build/
├── agent                              # Agent 主程序
├── cloudsecctl                        # 控制工具（make build-all 生成）
├── btf/                               # BTF 文件（内核兼容）
│   └── *.btf
└── plugins/
    ├── collector/
...
│   │       ├── container_dangerous_commands.yaml
│   │       ├── privilege_escalation_whitelist.yaml
│   │       ├── malicious_request_rules.yaml
│   │       ├── sensitive_file_rules.yaml
│   │       ├── file_monitor_whitelist.yaml
│   │       └── container_sensitive_file_rules.yaml
│   ├── nids/
│   │   ├── nids
│   │   └── config/
│   │       ├── nids.yaml
