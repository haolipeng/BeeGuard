...
本文档描述 Agent 与 server Server 联合运行的端到端测试流程：Agent 采集/检测数据 → 通过 gRPC 发送至 server → server 解析并写入 PostgreSQL → 查询数据库验证数据正确性。
...
| 对比项 | Standalone 模式 | 集成测试模式（本地 server） |
|--------|----------------|--------------------------|
| 服务端 | 不需要 | 需要本地启动 server |
| 数据库 | 不需要 | 需要本地 PostgreSQL |
| 数据输出 | stderr / 文件 | gRPC → server → PostgreSQL |
...
| vuln (server) | - | vuln_info | UPSERT | 漏洞信息，由 server 漏洞模块自动写入 |
| vuln (server) | - | host_vuln_scan_task | INSERT | 主机漏洞扫描任务，由 server 漏洞模块自动写入 |
| vuln (server) | - | host_vuln_detail | INSERT | 主机漏洞详情，由 server 漏洞模块自动写入 |
| vuln (server) | - | image_vuln_scan_task | INSERT | 镜像漏洞扫描任务，由 server 漏洞模块自动写入 |
| vuln (server) | - | image_vuln_detail | INSERT | 镜像漏洞详情，由 server 漏洞模块自动写入 |
...
- root 权限（Agent 运行需要）
- PostgreSQL 数据库(本地)
- 网络互通（Agent → server gRPC 端口 50051）
...
**重要**：`/opt/cloudsec/server/conf/server.yaml` 默认的数据库配置指向远程服务器，本地集成测试需修改为本地 PostgreSQL。
...
> **测试完成后务必恢复原配置**：`cp /opt/cloudsec/server/conf/server.yaml.bak /opt/cloudsec/server/conf/server.yaml`
...
Agent 连接后会自动下发 Scanner 扫描任务（扫描 `/root/scanner_test`），需先清除上次测试遗留的文件：
...
| 参数 | 说明 |
|------|------|
| `-config conf/server.yaml` | 指定配置文件（相对于 `/opt/cloudsec/server/`），包含数据库连接、gRPC/HTTP 端口等 |
...
- `数据库初始化失败` 错误 → 数据库连接异常，检查 2.2 数据库准备 和 2.3 修改server数据库配置
...
sudo ./bin/agent -config agent-local.yaml -test
```
...
| 参数 | 说明 |
|------|------|
| `-config agent-local.yaml` | 指定配置文件，包含 server 地址、插件目录等 |
...
**判定规则**：
- `connected to server` 出现 → Agent 与 server gRPC 连接成功
...
| `/opt/cloudsec/agent/logs/agent.log` | Agent 主程序日志持久化文件 |
| `/opt/cloudsec/agent/logs/plugins/ebpf_base_detector/ebpf_base_detector.log` | eBPF 插件日志 |
| `/opt/cloudsec/agent/logs/plugins/collector/collector.log` | Collector 插件日志 |
...
```bash
# 方式一：启动时过滤关键日志
sudo ./bin/agent -config agent-local.yaml -test 2>&1 | grep -E "(connected to server|plugin has been loaded|sync done|task sent|ERROR)"

# 方式二：保存全部输出到文件，在另一个终端搜索
sudo ./bin/agent -config agent-local.yaml -test 2>&1 | tee /tmp/agent_integration_test.log
# Terminal C 中搜索
...
Agent 启动后，server 会自动下发插件配置，collector 插件启动后立即执行首轮采集。等待约 30 秒后即可查询数据库。
...
> **前提**：eBPF 恶意请求检测依赖 DNS 事件捕获，需确保环境 DNS 解析正常。先执行 `dig minersns.com` 或 `nslookup minersns.com` 验证 DNS 可用。如果 DNS 不通（例如 `systemd-resolved` 超时），此测试无法进行。
...
**移除白名单方法**：编辑 `/opt/cloudsec/server/conf/server.yaml`，找到 `object_name: ssh` 对应的 task，将 `data` 字段中的 `"whitelist":["127.0.0.1","::1"]` 改为 `"whitelist":[]`，然后重启 server 和 Agent 使配置生效。
...
┌──────────────────────┐           ┌───────────────────────────────┐
│ id=1 Linux系统安全基线 │──┐       │ template_id=1, 检查SSH协议版本  │
│ item_count=3          │  ├─────→│ template_id=1, 检查密码最大天数  │
│                       │  │       │ template_id=1, 检查passwd权限   │
└──────────────────────┘  │       └───────────────────────────────┘
                          │
┌──────────────────────┐  │       ┌───────────────────────────────┐
│ id=2 SSH加固基线      │──┤       │ template_id=2, 禁止root远程登录 │
│ item_count=2          │  ├─────→│ template_id=2, SSH空闲超时检查   │
└──────────────────────┘  │       └───────────────────────────────┘
                          │
┌──────────────────────┐  │       ┌───────────────────────────────┐
│ id=3 文件完整性基线    │──┘       │ template_id=3, 检查/tmp是否存在 │
│ item_count=2          │  ──────→│ template_id=3, 检查hosts文件权限│
└──────────────────────┘          └───────────────────────────────┘
```

> `baseline_check_item.template_id` 关联 `baseline_template.id`，一个模板下有多个检查项。
...
INSERT INTO baseline_template (id, template_name, template_type, os_type, version, item_count, description, is_enabled)
...
检查项通过 `template_id` 关联所属模板，覆盖 agent 支持的主要检查类型。

```sql
-- ============================================================
-- 模板 1 的检查项（template_id=1，Linux 系统安全基线，共 3 项）
-- ============================================================

-- 1-1: command_check — 检查 SSH 协议版本
INSERT INTO baseline_check_item (template_id, item_name, category, risk_level, check_rules, fix_suggestion)
...
VALUES (1, '确保/etc/passwd权限为644或更严格', '文件权限', 'high',
  '{"condition":"all","rules":[{"type":"file_permission","param":["/etc/passwd","644"],"filter":"","require":"","result":"644"}]}',
  '执行 chmod 644 /etc/passwd');

-- ============================================================
-- 模板 2 的检查项（template_id=2，SSH 加固基线，共 2 项）
-- ============================================================
...
-- ============================================================
-- 模板 3 的检查项（template_id=3，文件完整性基线，共 2 项）
-- ============================================================
...
VALUES (3, '确保/etc/hosts权限为644', '文件完整性', 'medium',
  '{"condition":"all","rules":[{"type":"file_permission","param":["/etc/hosts","644"],"filter":"","require":"","result":"644"}]}',
...
> - `rules[].param`：参数数组，通常为命令或文件路径。`file_permission` 类型需要两个参数：`[文件路径, 权限值]`，如 `["/etc/passwd", "644"]`
...
```sql
-- 查看所有模板
SELECT id, template_name, template_type, item_count, is_enabled
FROM baseline_template ORDER BY id;

-- 查看每个模板下的检查项数量（应与 item_count 一致）
SELECT t.id AS template_id, t.template_name, t.item_count,
       COUNT(i.id) AS actual_items
FROM baseline_template t
LEFT JOIN baseline_check_item i ON i.template_id = t.id
GROUP BY t.id, t.template_name, t.item_count
ORDER BY t.id;

-- 查看所有检查项及其附属模板
SELECT i.id, i.template_id, t.template_name,
       i.item_name, i.category, i.risk_level
FROM baseline_check_item i
JOIN baseline_template t ON t.id = i.template_id
...
```bash
curl -X POST http://localhost:8081/api/baseline/check \
...
Agent 连接后，server 会自动下发目录扫描任务（扫描 `/root/scanner_test`，由 server.yaml 中 `data: '{"exe":"/root/scanner_test"}'` 控制）。在启动 Agent **之前**，先创建测试目录和 EICAR 标准测试文件：

```bash
# 创建专用测试目录并放入 EICAR 测试文件
mkdir -p /root/scanner_test
echo 'X5O!P%@AP[4\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*' > /root/scanner_test/eicar_test.com
```

启动 Agent 后，约 5 秒后 server 自动下发扫描任务，Scanner 插件会扫描 `/root/scanner_test` 目录并检出该文件。
...
验证要点：
- `file_path` 为 `/root/scanner_test/eicar_test.com`
...
```bash
# 创建多个测试文件（在自动扫描目录下）
echo 'X5O!P%@AP[4\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*' > /root/scanner_test/eicar_1.exe
echo 'X5O!P%@AP[4\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*' > /root/scanner_test/eicar_2.sh
```
...
WHERE agent_id = '123456' AND file_path LIKE '/root/scanner_test/eicar%';
```
...
> **重要**：执行前请确认 Nginx 正在运行（`systemctl is-active nginx`），否则 NIDS 无法捕获 HTTP 流量。如果 Nginx 已停止，执行 `sudo systemctl start nginx` 重新启动。
...
1. **Trivy 漏洞数据库**：server 启动时会自动从 OCI 仓库下载 Trivy DB（首次下载约 40MB），需确保服务器可访问 `ghcr.io`。
   ...
   > 如果网络不通，可在有网络的机器上手动下载 Trivy DB 文件，然后拷贝 `trivy.db` 和 `metadata.json` 到 `/opt/cloudsec/server/data/trivy-db/db/` 目录下。
...
2. **server.yaml 漏洞模块配置**：确认 `vuln.enabled` 为 `true`（默认已启用）。
   ...
   vuln:
     enabled: true
     db_dir: /opt/cloudsec/server/data/trivy-db
...
1. 启动 server → 漏洞模块初始化，下载/打开 Trivy DB
2. 启动 Agent → Collector 采集主机软件包写入 `asset_software`
3. server 启动约 **30 秒**后自动执行首次漏洞匹配
4. 匹配引擎读取 `asset_software` 中 dpkg/rpm 类型的软件包 → 逐包查询 Trivy DB → 将结果写入 `host_vuln_detail`

**server 日志观察**（Terminal A）：

```
# 漏洞模块初始化
INFO  [VulnDB] 漏洞数据库初始化成功: /opt/cloudsec/server/data/trivy-db/db/trivy.db
...
等待首次匹配完成后（观察 server 日志出现 `漏洞匹配任务完成`），查询数据库。
...
```
Collector Plugin                  server Server                       PostgreSQL
┌──────────────────┐  gRPC       ┌─────────────────┐  UPSERT        ┌──────────────────────┐
...
2. **server.yaml 中启用镜像和镜像软件包采集任务**：默认 `tasks` 中**未配置** DataType 5058 和 5059，需手动添加。
   ...
   > **重要**：修改后需重启 server 和 Agent 使配置生效。
...
5. server 启动约 **30 秒**后自动执行漏洞匹配（含镜像匹配）
...
# 终端 A：停止 server（Ctrl+C）
```

### 11.2 验证 Agent 离线

Agent 断开后，server 会更新连接状态：
...
# --- 恶意软件扫描测试产物 ---
rm -rf /root/scanner_test  # EICAR 测试文件目录
...
**排查：**
1. 确认 server 日志中有数据接收日志（非 error 级别）
2. 确认 server 数据库连接正常：检查启动日志无 `数据库初始化失败`
3. 检查 server 日志是否有写入错误：`写入失败` 关键字
...
1. 检查 server 日志中是否有 `handlePackagedData` 相关日志
