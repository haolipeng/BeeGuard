...
const (
	// 任务接收（控制台 → 插件）
	DataTypeDBUpdate   = datatype.ScannerDBUpdate // 病毒库更新
	DataTypeDirScan    = datatype.ScannerDirScan  // 指定目录扫描
	DataTypeFullScan   = datatype.ScannerFullScan // 全盘扫描

	// 结果上报（插件 → Agent → Server）
	DataTypeScanStatus = datatype.ScannerScanStatus // 扫描任务状态
	DataTypeFileDetect = datatype.ScannerFileDetect // 静态文件检出
)
