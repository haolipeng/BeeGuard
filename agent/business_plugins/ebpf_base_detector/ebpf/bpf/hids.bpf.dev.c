    ...
    struct bpf_raw_tracepoint_args *ctx)
{
    ...
    //~ 从Per-CPU Map获取缓冲区（在 subprogram 内部获取，保持 MAP_VALUE 指针类型信息）
    struct execve_event *evt = bpf_map_lookup_elem(&percpu_buf, &key);
    if (!evt)
        return 0;

    //~ 初始化事件仅清零条件赋值的字段，大数组由各 helper 函数内部清零
    ...
    //~ 通过 i_mode + socket family 推导 fd_type
    //~ socket 伪文件系统 (sockfs) 的 dentry 是 disconnected 的（d_parent 指向自身），
    //~ build_d_path 无法为 socket FD 构建路径（返回 path_len=1），因此不能依赖路径字符串。
    //~ 这里直接检查 inode 的 i_mode 文件类型位（S_IFSOCK = 0140000），
    //~ 并进一步检查 socket family 是否为 AF_INET(2) 或 AF_INET6(10)，
    //~ 排除 AF_UNIX(1) 等非网络 socket（如 Node.js IPC 使用的 Unix domain socket）
    {
    ...
    //~ 读取控制终端名称（内联，避免独立 __noinline 子程序）
    ...
    //~ 扫描当前进程 FD 0-15，寻找已连接的 IPv4 socket，
...
static __noinline int query_dns_record(char *data, int data_len, char *domain, int domain_size, __u16 *query_type)
{
    ...
    int read_pos = 12;     //~ DNS 查询域名起始位置
    int write_pos = 0;     //~ 域名输出写入位置
    int remain = 0;        //~ 当前标签剩余字符数
    int first_label = 1;   //~ 是否第一个标签

    //~ 状态机解析域名：逐字节读取，处理长度前缀编码
    #pragma unroll
    for (int i = 0; i < 64; i++) {
...
static __noinline int handle_inode_create(struct pt_regs *ctx)
{
    ...
    //~ 提取 dentry（security_inode_create 的第 2 个参数）
...
static __noinline int handle_inode_rename(struct pt_regs *ctx)
{
    ...
    //~ 提取 old_dentry（参数2）和 new_dentry（参数4）
    ...
    //~ 获取文件系统 ID（从 new_dentry）
    query_s_id_by_dentry(evt->s_id, new_de);

    //~ 使用 percpu_file_path_buf 构建旧路径
    ...
    //~ 使用 percpu_path_buf 构建新路径（复用现有的 per-CPU map，同一 CPU 不会冲突）
